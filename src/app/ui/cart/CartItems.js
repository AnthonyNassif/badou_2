...
const CartItems = () => {
  ...
  return (
    ...
    <div className="w-full max-w-xs bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
      <h2 className="text-2xl text-amber-600 pb-4">{cartData.length} Item(s) in Cart</h2>
      <ul>
      {
        cartData.map((item) => (
          <li 
            key={item.id}
            className="p-2 m-2 rounded-full flex bg-orange-500">
            <div className="bg-amber-700 text-white text-center h-[30px] w-[30px] rounded-full pt-1">{item.type === "Buy" ? 'B' : 'R'}</div> 
